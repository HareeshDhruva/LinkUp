              ...
              <div className="p-1 text-center font-bold">
                <div>
                  {conversation?.map((friend,index) => (
                    <div className={`flex gap-2 text-center m-1 ${selectedConversation?._id === friend._id? "bg-black text-[#fff]": "bg-white text-black"} items-center p-1 rounded-3xl jusuutify-between`} key={index} onClick={()=>selectUSer(friend)}>
                      <div className="flex md:gap-5 items-center justify-between w-full">
                        {friend.profileUrl ? (
                          <img
                        ...
                        <p>{onlineUsers?.includes(friend._id) ? 'Online' : 'Offline'}</p>
                        <div className="text-[1.5rem] max-lg:text-[0.8rem] mx-2 max-md:hidden">
                          <MdChat />
                        </div>
                      </div>
                    </div>
        ...
        <div className="flex justify-between min-h-[8dvh] mx-4">
          <div className="flex md:gap-5 items-center justify-between w-full text-[#fff]">
            <div className="flex gap-4 justify-center items-center">
              ...
              <p className="max-md:text-[12px]">{selectedConversation?.username}</p>
            </div>
        ...
        <div className="col-span-3 bg-black overflow-y-scroll h-[85dvh] overflow-x-hidden max-sm:text-[0.5rem]">
              ...
              {item.senderId !== authUser._id ? (
                <>
                  ...
                  <div className="flex justify-start text-xs text-[#fff] gap-4 mx-4">
                    ...
                    <p className="text-[0.5rem]">{dateConvert(item.createdAt)}</p>
                  </div>
                </>
              ) : (
                <>
          ...
          <div className="fixed bottom-0 w-[70%] p-2">
            <div className="flex items-center md:mx-6">
              <div className={`flex gap-5 w-full justify-between items-center ${!selectedConversation ? "hidden":null}`}>
                <input
                  type="text"
                ...
                <div className="flex justify-center items-center">
                  <MdSend className="h-10 cursor-pointer text-center text-[30px] max-md:h-5 text-[#fff]" onClick={handleSubmitMessage}/>
                </div>
              </div>
            </div>
          </div>
